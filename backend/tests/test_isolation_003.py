...
def test_003_new_tables_workspace_scoped(client):
    ...
    db.add_all(rows)
    db.flush()
    exam_asg = db.query(Assignment).filter(Assignment.workspace_id == WS_EXAM, Assignment.title == "exam hw").one()
    lang_asg = db.query(Assignment).filter(Assignment.workspace_id == WS_LANG, Assignment.title == "lang hw").one()
    ...
    exam_set = db.query(PracticeSet).filter(PracticeSet.workspace_id == WS_EXAM, PracticeSet.title == "exam set").one()
    lang_set = db.query(PracticeSet).filter(PracticeSet.workspace_id == WS_LANG, PracticeSet.title == "lang set").one()
    ...
    exam_plan = db.query(Plan).filter(Plan.workspace_id == WS_EXAM, Plan.name == "exam plan").one()
    lang_plan = db.query(Plan).filter(Plan.workspace_id == WS_LANG, Plan.name == "lang plan").one()
