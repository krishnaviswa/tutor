...
CONTRACTS: dict[str, dict] = {
    ...
    "cohort-builder": {
        ...
        "ports": [],
        "timelineEvents": [],
        "shows": "Batches / groups / 1-on-1 pairings, invite token and waitlist. Topic taxonomy is tenant-defined.",
    },
    "parent-link": {
    ...
    "parent-home": {
        ...
        "ports": [],
        "timelineEvents": [],
        "shows": "Guardian hub for linked children only: attendance, practice, test, fee due if this guardian can see fees, activity, marksheet, receipts, teacher chat.",
    },
    "schedule": {
        "apis": ["GET /api/v1/sessions", "POST /api/v1/sessions", "PATCH /api/v1/sessions/{id}", "GET /api/v1/availability"],
        "entities": ["scheduled_sessions", "cohorts", "staff_availability"],
        "ports": ["calendar_video", "whatsapp"],
        "timelineEvents": ["session.scheduled", "session.rescheduled", "session.cancelled"],
        "shows": "Week calendar of sessions. Conflict detection on overlap. Cancelled sessions stay struck-through. Optional overlay shades hours outside the selected staff member's availability.",
    },
    "sessions": {
        "apis": ["GET /api/v1/sessions", "POST /api/v1/sessions", "PATCH /api/v1/sessions/{id}", "GET /api/v1/students"],
        "entities": ["scheduled_sessions", "students", "cohorts"],
        "ports": ["whatsapp"],
        "timelineEvents": ["session.scheduled", "session.cancelled"],
        "shows": "Upcoming / Completed / Cancelled tabs over every session, filterable by an added student. Completed is time-driven (Auto completed until a record is filed, then Staff completed). 1-on-1 sessions are booked here against a roster student.",
    },
    "availability": {
        "apis": ["GET /api/v1/availability", "PUT /api/v1/staff/{id}/availability", "GET /api/v1/staff"],
        "entities": ["staff_availability", "staff_memberships"],
        "ports": [],
        "timelineEvents": [],
        "shows": "Per-staff weekly available windows plus one-off date exceptions (time off or an extra slot). Staff edit their own; owner sees and edits anyone. Feeds the schedule overlay and booking checks.",
    },
    "session-pre": {
    ...
    "live-teacher": {
        ...
        "ports": ["calendar_video"],
        "timelineEvents": [],
        "shows": "Teacher console around the mock video session, with in-app chat and MCQ polls (not Meet events).",
    },
    "live-student": {
        ...
        "ports": ["calendar_video"],
        "timelineEvents": [],
        "shows": "Student live view / waiting overlay, with in-app chat (not Meet events).",
    },
    "session-video": {
    ...
    "record": {
        ...
        "shows": "Attendance, notes, board photos, local capture of chat/poll events. Fan-out to each attendee timeline.",
    },
    "library": {
        ...
        "ports": ["storage"],
        "timelineEvents": [],
        "shows": "Lessons with kind, duration, progress, playlist, drip, and views. Visibility per cohort. Storage stays local.",
    },
    "lesson": {
        ...
        "shows": "Single lesson: notes, duration, next practice. Progress writes to the record.",
    },
    "assign-issue": {
        ...
        "shows": "Issue homework with due date, rubric, and resubmit flag.",
    },
    "assign-grade": {
        ...
        "shows": "Submit, grade, return. Late and resubmit counts on the row.",
    },
    "qbank": {
        ...
        "ports": ["storage"],
        "timelineEvents": [],
        "shows": "Item bank tagged by tenant taxonomy, with difficulty lanes and usage counts — not a hardcoded syllabus.",
    },
    "practice-build": {
        ...
        "shows": "Same-day or scheduled practice set from tags, hand-pick, or auto-assemble by difficulty.",
    },
    "practice-play": {
    ...
    "test-setup": {
        ...
        "shows": "Timed paper setup with sections and optional negative marking.",
    },
    "test-runner": {
        ...
        "shows": "Student test runner with timer, question palette, and resume.",
    },
    "analysis": {
    ...
    "messages": {
        ...
        "shows": "In-platform threads with unread and local attachments so the record exists. WhatsApp may notify.",
    },
    "announce": {
        ...
        "shows": "One-to-cohort broadcast with scheduled_at and stored channel matrix. Send stays mock. Parent WhatsApp if opted in.",
    },
    "timeline": {
        ...
        "ports": [],
        "timelineEvents": [],
        "shows": "Append-only event stream for one student. Filter by event_type; export=1 returns a slice. The ledger.",
    },
    "notif-prefs": {
    ...
    "student-dash": {
        ...
        "ports": [],
        "timelineEvents": [],
        "shows": "Named next session (title + time), due practice unanswered/total, this-week test, doubt outcome, weak tags.",
    },
    "teacher-dash": {
        ...
        "ports": [],
        "timelineEvents": [],
        "shows": "Cohort health: attendance %, weekday bars, practice completion by set, doubt backlog, named chase list.",
    },
    "owner": {
        ...
        "ports": [],
        "timelineEvents": [],
        "shows": "Operating scorecard (sessions, students, practice %, SLA, revenue, churn) plus quota bars (80% / 100%). Sign-in methods and G1 preview. Feature kill switches.",
    },
    "reports": {
        ...
        "shows": "Term marksheet from the ledger: attendance, practice %, latest test, teacher note. CSV export. Parent WhatsApp when a report is ready.",
    },
    "mentor": {
    ...
    "billing": {
        ...
        "shows": "Student fee plans and invoices with auto-issue, coupons, and proration (tenant collecting from learners). Checkout stays mock.",
    },
    "payments": {
        ...
        "shows": "Due amount + date, receipt history, paid / pending / overdue. Mock checkout.",
    },
    "subscription": {
    ...
    "payouts": {
        ...
        "ports": ["payments_platform"],
        "timelineEvents": [],
        "shows": "Multi-teacher payout statements (name, period, session count) without Stripe.",
    },
    "audit": {
    ...
    "automation": {
        ...
        "ports": [],
        "timelineEvents": [],
        "shows": "Opt-in when → then rules. Missed-practice rules write backlog items. Opinion as choice, not law.",
    },
    "integrations": {
...
def modules() -> list[dict]:
    return [
        ...
        {"id": "B1", "name": "Scheduling & calendar", "dependsOn": ["A5"], "phase": 1, "alwaysOn": False, "entities": ["scheduled_sessions", "staff_availability"]},
